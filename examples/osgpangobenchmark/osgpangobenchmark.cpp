...
{
public:
    ConvertPangoText(const std::string& filename): osg::NodeVisitor(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN)
    {
        _numImages = 0;
        _filenamebase = osgDB::getNameLessExtension(filename);
    ...
    void apply(osg::Node& node)
    {
        ...
        if (node.getStateSet())
        {
            ...
            OSG_NOTICE<<"Node "<<node.className()<<" has StateSet"<<std::endl;
            new_node->setStateSet(node.getStateSet());

            _group->addChild(new_node);
        }
            ...
            ++titr)
        {
                ...
                ++aitr)
            {
                ...
                if (texture)
                {
                    ...
                    if (image && _images.count(image)==0)
                    {
                        _images.insert(image);

                        std::stringstream sstr;
                        sstr<<_filenamebase<<"_"<<_numImages<<".tif";
                        ++_numImages;
                        ...
                        OSG_NOTICE<<"Image "<<image<<" write : "<<filename<<std::endl;
                    }
                }
            }
        }
    ...
    typedef std::set<osg::Image*> Images;
...
int main(int argc, char** argv) {
	...
	std::string outputFilename;
        while(args.read("-o", outputFilename)) {}

        std::string outputConvertedSubgraphFilename;
        while(args.read("--oc", outputConvertedSubgraphFilename)) {}
	...
	if(!t->finalize()) return 1;
        ...
        {
            osgDB::writeNodeFile(*t, outputFilename);
            return 0;
        }

        if (!outputConvertedSubgraphFilename.empty())
        {
            ConvertPangoText convertPangoText(outputConvertedSubgraphFilename);
            t->accept(convertPangoText);

            osg::ref_ptr<osg::Node> node = convertPangoText.getSubgraph();
            osgDB::writeNodeFile(*node, outputConvertedSubgraphFilename);
